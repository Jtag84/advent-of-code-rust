...
fn part2(file_input_path: &str) -> usize {
    ...
    grid.indexed_iter()
        ...
        .map(|coordinates| vec![
            vec![coordinates.north_west(), Some(coordinates), coordinates.south_east()],
            vec![coordinates.north_east(), Some(coordinates), coordinates.south_west()]])
        .map(|coords| coords
            .into_iter().map(|coords: Vec<Option<GridCoordinates>>| coords.iter()
                .map(|optional_coordinate| optional_coordinate
                    .map(|c| grid.get(c.row(), c.column())).flatten()).collect()).collect())
